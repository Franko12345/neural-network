...
- [ ] Old `update(nn, X, y_int, epoch, loss, acc, dataset_name)` is
      removed (v1 visualizer code is rewritten, not extended)
- [ ] Panel switching via `set_panel(name)` (called by main.py)
- [ ] Backwards-compatible smoke: a tiny adapter in v1's `__main__`
      block passes the old kwargs into the new dict form, exits 0
- [ ] Headless smoke: `SDL_VIDEODRIVER=dummy python3 -c "from
      visualizer import Visualizer; ..."` opens, renders 10 frames,
      exits 0
- [ ] ponytail: one method per public action; metrics dict keys are
      flat strings; no nested state
- [ ] v1 tests still pass (sanity — visualizer doesn't directly
