...
- [ ] `requirements.txt` gains `gymnasium>=0.29` (v1 install path still
      works without it; v2 install pulls it in)
- [ ] README updated with new section: "MountainCar + Transformer
      (v2)" showing 2-3 new GIFs in a second visual table
- [ ] Tag `v0.2.0` on the merge commit, push tag
...
- [ ] `CONTEXT.md`: status flipped from "Shipped (v1)" → "v2 ready";
      roadmap v1 → implemented, v2 → implemented
- [ ] v1 spec at `.scratch/nn-visualizer/README.md` stays
      `status: implemented` (no change)
- [ ] v2 spec at `.scratch/nn-v2-future/README.md` frontmatter flipped
      `status: ready-for-agent` → `status: implemented` (per Matt
      Pocock pitfall: spec not marked "Implementada" after parent
      closes)
- [ ] `.scratch/nn-v2-future/issues/README.md` tickets table all
      flipped to `implemented`
- [ ] Final commit message references parent spec