...
- [x] `tests/test_modules.py`: LayerNorm forward/backward correctness;
      Residual gradient split
- [x] `tests/test_optim.py`: AdamW bias correction at step 0;
      weight decay decoupled from gradient
- [x] Headless smoke: each module runs standalone, exits 0
- [x] ponytail: no flag on `step()`; one module per file
